...
const caseBrands = [
  { name: "MTE", image: "/brands/mte.png" },
  { name: "Vertem Engagement", image: "/brands/vertem-engagement.webp", legacy: "anteriormente LTM", darkMark: true },
  ...
  { name: "Stanley", image: "/brands/stanley.png", darkMark: true },
    ...
    <div className="from-data-site">
      ...
      <main>
        ...
        <section className="fd-company-proof" aria-labelledby="company-proof-title">
          ...
          <div className="fd-logo-wall fd-case-wall">{caseBrands.map((company) => <article className={"darkMark" in company && company.darkMark ? "fd-dark-mark" : undefined} key={company.name}><div className="fd-logo-plate"><Image src={company.image} alt="" aria-hidden="true" width={190} height={72} unoptimized /></div><span className="fd-brand-name">{company.name}</span>{"legacy" in company && company.legacy ? <small className="fd-brand-legacy">{company.legacy}</small> : null}</article>)}</div>
